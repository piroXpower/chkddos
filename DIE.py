...
TELEGRAM_BOT_TOKEN = '8748687428:AAFhKod8wAgg2wUCCcYxyrlA-tHw9TW8634'
ADMIN_USER_ID = 8617782892
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*Welcome To Hitler Bots, Make Sure Be Polite! 🔥*\n\n"
        "*Bot is only for educational purposes and do not support any illigal org or illigal contents🔥*\n\n"
        "*Use /run <ip> <port> <duration>*\n"
        "*Let the war begin! ⚔️💥 any issue dm @BeMySugarBaby*"
    )
...
async def manage(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Are laude, @BeMySugarBaby se permission leke aa madarchod 🌚😂.*", parse_mode='Markdown')
        return

    if len(args) != 2:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /sudo <add|rem> <user_id>*", parse_mode='Markdown')
        return
...
def main():
    ...
    application.add_handler(CommandHandler("sudo", manage))
    application.add_handler(CommandHandler("run", attack))
